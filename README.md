...
每次本地修改笔记或新增图片后，直接双击根目录中的 `upload_to_github.bat`。脚本会自动暂存全部改动、提交、同步远端并推送当前分支。

也可以在命令行中提供自定义提交说明：

```powershell
.\upload_to_github.bat "本次更新说明"
```

上传时不需要运行 `stage_docs.py`。Docker 构建网站时会自动整理 MkDocs 文件；该脚本仅用于本地预览。提交前建议先看一眼 `git status`，确认本次改动都是要上传的内容。
